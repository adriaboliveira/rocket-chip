...
class TLDebugModuleInner(device: Device, getNComponents: () => Int, beatBytes: Int)(implicit p: Parameters) extends LazyModule
{
  ...
  val customNodeOpt = cfg.custom.map{ c => BundleBridgeSource(() => {new DecoupledIO(new DebugCustom(c))})}
  ...
  lazy val module = new LazyModuleImp(this){
    ...
    cfg.custom.foreach { c =>
      ...
      val custom_data = custom.bits.data.toBools
      val custom_bytes =  Seq.tabulate(c.width/8){i => custom_data.slice(i*8, (i+1)*8).asUInt}
      when (custom.fire()) {
        (abstractDataMem zip custom_bytes).foreach {case (a, b) =>
          a := b
        }
      }
    }
    ...
    customNodeOpt.foreach { c =>
      c.bundle.bits.addr  := accessRegisterCommandReg.regno
    ...
    when (ctrlStateReg === CtrlState(Waiting)){
      ...
      if (customNodeOpt.isDefined) {
        when (customNodeOpt.get.bundle.fire()) {
